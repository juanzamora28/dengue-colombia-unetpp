...
def post_process(pred_np, k=3):

    struc = np.ones((k, k), dtype=bool)
    out = pred_np.copy()
    for cls in (1, 2, 3):
        mask_cls = (pred_np == cls)
        closed   = binary_closing(mask_cls, structure=struc)
        out[closed & ~mask_cls] = cls
    return out
...
S1_POS_DILATE_ITERS = 0  # ← usa el mismo valor que en entrenamiento
...
base_cfg = {
    ...
    # ── NUEVO: Superficies Construidas (anual, % 0..100) ─────────────────────
    ...
    "luznoct_scale_to_unit": False,      # si quieres 0..1 con p99 → True
...
base_cfg.update({
    ...
    "use_habitat_index": False,                  # actívalo en el cfg que corras
    ...
    "use_ntl_anomaly": False,                    # actívalo en el cfg que corras
    ...
    "use_exo_interactions": False,               # actívalo en el cfg que corras
...
# Base común: P/T actuales + incidencia previa one-hot (4 lags) y TODO lo inventado APAGADO
_cfg_base_real = {
    **base_cfg,
...
cfg_ORIGINAL_RAW_PTSTACKS = {
    **cfg_ORIGINAL_RAW,
    ...
    # Activa stacks de P/T de semanas pasadas (lags explícitos como canales)
    "use_pt_stacks": True,
    "p_stack_weeks": 4,   # t-1..t-4 de Precipitación
    "t_stack_weeks": 4,   # t-1..t-4 de Temperatura
...
# ──────────────────────────────────────────────────────────────────────────────
# 3) CLASE DengueDataset (incluye Incidencia t-1 opcional "mixta")
# ──────────────────────────────────────────────────────────────────────────────
class DengueDataset(Dataset):
    def __init__(
        ...
        t_avg_weeks=4,
        degday_tau_q=0.40,
        # ── NUEVO: Hábitat H(x) y NTL-anomaly E(x,t) e interacciones ──
        use_habitat_index=False,
...
def select_pair(index, a, b, min_a=0.08, min_b=0.08,
                max_purity=0.98, target=300, per_file_cap=10):

    out, used = [], {}
    cands = [p for p in index
             if p.fracs[a] >= min_a and p.fracs[b] >= min_b and p.purity <= max_purity]
    # prioriza equilibrio entre a y b
    cands.sort(key=lambda p: -min(p.fracs[a], p.fracs[b]))
    for p in cands:
        k = (p.basename, p.dom)
        if used.get(k, 0) >= per_file_cap: continue
        out.append(p); used[k] = used.get(k,0)+1
...
def main():
    ...
    def train_binary_end_to_end(
            ...
            *,
            ...
            p_minor_fixed=None,                # igual que usabas en plan A
            p_minor_warm=12,                    # sin schedule por defecto
        ...
        ce_bin = torch.nn.CrossEntropyLoss(
            weight=torch.tensor(class_weights_bin, device=device),
            label_smoothing=0.05,         # << NUEVO: suaviza targets 5%
            ignore_index=-100,
        )
        ...
        def _sched_p_minor(ep):
            ...
            # decae de 0.65 → 0.50 durante el warm-up
            return 0.60 + (0.35 - 0.60) * t
    ...
    # ──────────────────────────────────────────────────────────────────────────
    # FUNCS de binarización para CALIBRACIÓN (mismas que en training de cada stage)
    # ──────────────────────────────────────────────────────────────────────────
    ...
    def _bin_targets_s1_0vR(y, m):
        ...
        # Dilatación 3x3 S1_POS_DILATE_ITERS veces (si >0)
        iters = int(S1_POS_DILATE_ITERS)
        if iters > 0:
            rn = resto.cpu().numpy().astype(bool)
            selem = np.ones((3,3), dtype=bool)
            for i in range(rn.shape[0]):
                rn[i] = binary_dilation(rn[i], structure=selem, iterations=iters)
            resto = torch.from_numpy(rn).to(t.device)
    ...
    def visualize_cascade_maps(dataset, predict_fn, device, save_dir):
        """
        Versión BINARIA (0 vs RESTO). 
        Espera que `predict_fn` reciba un tensor X de shape [1,C,H,W] y devuelva:
        - logits [1,2,H,W] (preferido), o 
        - prob-resto [1,1,H,W] / [1,H,W].
        Pintamos: -1 (fuera) / 0 / 1.
        """

        os.makedirs(save_dir, exist_ok=True)

        # Colores: [-1, 0, 1] = [Fuera de área, clase0, RESTO(1-2-3)]
        colors = ['#808080', '#000080', '#FFAA00']
        cmap = ListedColormap(colors)
        # ticks en -1,0,1 ; los cortes van a mitad
        norm = BoundaryNorm([-1.5, -0.5, 0.5, 1.5], ncolors=cmap.N, clip=True)
        ...
        cat_labels = ["Out of Area", "Class 0", "Class 1"]

        dept_name = getattr(dataset, 'dept_name', None)
        dept_tag = f" (dept: {dept_name})" if dept_name else ""
        ...
        for idx in range(len(dataset)):
            ...
            with torch.no_grad():
                ...
                # Acepta logits [1,2,H,W] o prob-resto [1,1,H,W] / [1,H,W]
                if isinstance(out, torch.Tensor):
                    if out.ndim == 4 and out.shape[1] == 2:
                        # logits → prob RESTO
                        p_rest = torch.softmax(out, dim=1)[:, 1]
                    else:
                        # prob RESTO directo
                        p_rest = out
                        if p_rest.ndim == 4 and p_rest.shape[1] == 1:
                            p_rest = p_rest[:, 0]  # [1,H,W]
                    pred_bin = (p_rest >= 0.5).long().squeeze(0).cpu().numpy()  # [H,W] en {0,1}
                else:
                    raise RuntimeError("predict_fn debe devolver un torch.Tensor")

            # Etiqueta GT a binario: 0→0; 1/2/3→1; 255→-1 (fuera/ignore)
            y_np = y.numpy().astype(np.int16, copy=True)
            y_bin = np.where(y_np == 255, -1, np.where(y_np == 0, 0, 1)).astype(np.int16)

            # Enmascara pred fuera del área: donde GT es -1, pred = -1
            pred_bin = pred_bin.astype(np.int16, copy=True)
            pred_bin[y_bin == -1] = -1

            # Recorte a bbox de válidos (para foco visual)
            valid = (y_bin != -1)
            if valid.any():
            ...
            im0 = axes[0].imshow(y_plot, cmap=cmap, norm=norm, origin='upper')
            axes[0].set_title("Original Map")
            axes[0].axis('off')

            im1 = axes[1].imshow(pred_plot, cmap=cmap, norm=norm, origin='upper')
            axes[1].set_title("Binary Prediction")
            axes[1].axis('off')

            # Barra de color común
            cax = fig.add_axes([0.2, 0.02, 0.6, 0.03])
            cb = plt.colorbar(im0, cax=cax, orientation='horizontal', ticks=[-1, 0, 1])
            ...
            plt.close(fig)
            print(f"→ Binary map guardado: {out_path}")
    ...
    for period in PERIODS:
        ...
        for feat_name, feat_cfg in FEATURE_CONFIGS:
            ...
            expdir = os.path.join(out_base, f"{feat_name}__GLOBAL")
            ...
            mix_end = MixedBatchIterator(loader_minor_combined, loader_std, p_minor=0.50, steps_per_epoch=STEPS_PER_EPOCH)

            # Dilatación S1 fija a 0 (no “limpieza” de positivos aquí)
            global S1_POS_DILATE_ITERS
            S1_POS_DILATE_ITERS = 0
            ...
            # (usa compute_split_class_freqs si ya la tienes; f[0]=freq clase 0; resto=1-f0)
            train_freq = compute_split_class_freqs(train_ds)  # np.array([f0,f1,f2,f3])
            ...
            # Pesos de clase BINARIOS a partir de la frecuencia del TRAIN (con shrink hacia uniforme)
            train_freq = compute_split_class_freqs(train_ds)  # np.array([f0,f1,f2,f3])
            f0  = float(train_freq[0])
            fR  = max(1e-6, 1.0 - f0)
            w0  = 1.0 / max(1e-6, f0)
            ...
            # >>> shrink para evitar sobreajuste al prior de RESTO
            ALPHA = 0.35  # 0.6 de tus pesos inversos + 0.4 hacia [0.5, 0.5]
            bin_weights = [
                ALPHA*bin_weights[0] + (1-ALPHA)*0.5,
                ALPHA*bin_weights[1] + (1-ALPHA)*0.5,
            ]
            # renormaliza por seguridad
            s = bin_weights[0] + bin_weights[1]
            bin_weights = [bin_weights[0]/s, bin_weights[1]/s]
            ...
            trained_bin = train_binary_end_to_end(
                ...
                threshold=0.5,
                s1_keep_from_lag=3, s1_smooth_kernel=1, s1_dropout_p=0.35, s1_scale=0.60,
                log_prefix="[BIN S1]"
            )
            ...
            scaler_bin = calibrate_temperature_binary_head(
                trained_bin, val_loader, DEVICE, build_targets_fn=_bin_targets_s1_0vR,
                max_iter=200, lr=0.05,
                preprocess_X=(lambda X: restrict_prev_for_s1_inplace(
                    X, val_ds, zero_t1=True, keep_from_lag=3, smooth_kernel=1, dropout_p=0.0, scale=0.60))
            )
            ...
            def _pre_s1_val(Xin):
                 Xs = Xin.clone()
                 restrict_prev_for_s1_inplace(Xs, val_ds, zero_t1=True, keep_from_lag=3, smooth_kernel=1, dropout_p=0.0, scale=0.60)
                 return Xs

            def _pre_s1_test(Xin):
                Xs = Xin.clone()
                restrict_prev_for_s1_inplace(Xs, test_ds, zero_t1=True, keep_from_lag=3, smooth_kernel=1, dropout_p=0.0, scale=0.60)
                return Xs
            ...
            # Liberar VRAM del modelo binario de este experimento
            try:
                trained_bin.to("cpu")
            except Exception:
                pass
...
if __name__ == '__main__':
    # Rutas base
    DATA_DIR    = r'C:\Users\juanz\Desktop\UNET_DENGUE'
    SHAPE_PATH  = os.path.join(DATA_DIR, "Shape", "Shape", "Municipios", "Municipios.shp")
    PERIODS     = [#"2007-2019",
                   #"2016-2019",
                    "2018-2019",
                   ]
    OUTPUT_DIR  = os.path.join(DATA_DIR, "experiments_results_dengue_binario_prueba2")
    ...
    # --- LEEMOS EL SHAPE SÓLO UNA VEZ EN EL PROCESO PRINCIPAL ---
    ...
    # También guardamos muni en global si lo usa visualize_test_batch
    muni_gdf = muni.copy()
